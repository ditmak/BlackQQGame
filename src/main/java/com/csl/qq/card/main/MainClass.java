...
public class MainClass {
	...
	public static void getCurrentInfo(){
		...
		String url = "http://mfkp.qzapp.z.qq.com/qshow/cgi-bin/wl_card_box?sid=AfYnBGlvxgtzr4um2cL44G-f";
		String data = getUrlContent(url);
		Pattern pattern = Pattern.compile("card=(\\d+).*?slot=(\\d+)",Pattern.DOTALL);
	...
	public static void getSuperCardID(){
		...
		Pattern pattern = Pattern.compile("\\d\\. (.*?)\\[.*?;id=(\\d+)",Pattern.DOTALL);
		for (Theme theme : list) {
			String url = "http://mfkp.qzapp.z.qq.com/qshow/cgi-bin/wl_card_strategy?sid=AfYnBGlvxgtzr4um2cL44G-f&fuin=0&steal=0&themeid="+theme.getThemeID();
			String content = getUrlContent(url);
			Matcher matcher=pattern.matcher(content);
			while(matcher.find()){
				Card card = cardService.findCardByName(matcher.group(1),theme.getId());
	...
	private static void add(HashMap<String,Formula> mapf,HashMap<String, Integer> maps,String key){
	...
	public static void getSuperCardInfo() {
		...
		for (Theme theme : list) {
			// 2.拼凑连接
			String url = "http://mfkp.qzapp.z.qq.com/qshow/cgi-bin/wl_card_theme_info?sid=ARltUZg17dy3aN6LSTvOWXBZ&themeid="
					+ theme.getThemeID();
	...
	public static void getCardInfo() {
		...
		for (Theme t : list) {
			String url = "http://mfkp.qzapp.z.qq.com/qshow/cgi-bin/wl_card_theme_info?sid=ARltUZg17dy3aN6LSTvOWXBZ&price=10&themeid="
					+ t.getThemeID();
			String content = getUrlContent(url);
	...
	public static Integer getThemesInfo(Integer level, Integer pageno)
			 {
		String url = "http://mfkp.qzapp.z.qq.com/qshow/cgi-bin/wl_card_theme_list?sid=ARltUZg17dy3aN6LSTvOWXBZ&fuin=1&steal=0&refine=0&pageno="
				+ pageno + "&level=" + level;
